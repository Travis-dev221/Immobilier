...
BASE = Path(r"c:\Users\dia9a\OneDrive\Bureau\Location")
FILES = [
    BASE / "index.html",
    BASE / "vacances.html",
    BASE / "contact.html",
...
<div class="lead-modal" id="leadModal" onclick="if(event.target===this)closeLeadModal()">
  <div class="lead-box">
    ...
    <p class="lead-eyebrow">Pre-reservation WhatsApp</p>
    <h2 class="lead-title">Avant de continuer sur WhatsApp</h2>
    <p class="lead-sub">Le client remplit ici ses informations avant l'ouverture de WhatsApp. Vous recevez donc un message plus complet avec les coordonnees, les dates et les preferences.</p>
    <div class="lead-summary">
      <div><span>Bien concerne</span><strong id="leadContext">Demande generale</strong></div>
      <div><span>Budget indicatif</span><strong id="leadPrice">A confirmer</strong></div>
    </div>
    <div class="lead-grid">
      <div class="lead-field"><label for="leadFirstName">Prenom</label><input type="text" id="leadFirstName" placeholder="Votre prenom"></div>
      <div class="lead-field"><label for="leadLastName">Nom</label><input type="text" id="leadLastName" placeholder="Votre nom"></div>
      <div class="lead-field"><label for="leadPhone">Numero de telephone</label><input type="tel" id="leadPhone" placeholder="77 000 00 00"></div>
      ...
      <div class="lead-field full"><label for="leadAddress">Adresse</label><input type="text" id="leadAddress" placeholder="Votre adresse complete"></div>
      <div class="lead-field"><label for="leadProjectType">Type de demande</label><select id="leadProjectType"><option>Location vacances</option><option>Location longue duree</option><option>Achat immobilier</option><option>Local commercial</option><option>Renseignement general</option></select></div>
      <div class="lead-field"><label for="leadPaymentMethod">Methode de paiement souhaitee</label><select id="leadPaymentMethod"><option>Wave</option><option>Orange Money</option><option>Virement bancaire</option><option>Carte bancaire</option><option>Especes</option><option>A definir</option></select></div>
      <div class="lead-field lead-rental-only"><label for="leadStartDate">Date d'arrivee</label><input type="date" id="leadStartDate"></div>
      <div class="lead-field lead-rental-only"><label for="leadEndDate">Date de depart</label><input type="date" id="leadEndDate"></div>
      <div class="lead-field lead-rental-only"><label for="leadNights">Nombre de jours</label><input type="number" id="leadNights" min="1" readonly placeholder="Calcule automatiquement"></div>
      ...
      <div class="lead-field lead-rental-only"><label for="leadChef">Option chef cuisinier</label><select id="leadChef"><option>Oui</option><option>Non</option><option>A confirmer</option></select></div>
      <div class="lead-field"><label for="leadRef">Reference</label><input type="text" id="leadRef" readonly></div>
    ...
    <p class="lead-note" id="leadNote">Aucune donnee n'est stockee sur le site. Le formulaire sert uniquement a preparer votre message WhatsApp.</p>
...
var leadData={context:'Demande generale',amount:0,category:'Renseignement general',ref:''};
function makeLeadRef(){var d=new Date(),p=function(n){return String(n).padStart(2,'0')},r=Math.random().toString(36).slice(2,6).toUpperCase();return 'WA-'+d.getFullYear()+p(d.getMonth()+1)+p(d.getDate())+'-'+r;}
...
function normalizeLeadCategory(v){var allowed=['Location vacances','Location longue duree','Achat immobilier','Local commercial','Renseignement general'];return allowed.indexOf(v)>=0?v:'Renseignement general';}
function isRentalLeadCategory(v){return v==='Location vacances'||v==='Location longue duree';}
...
function updateLeadRentalFields(){var category=document.getElementById('leadProjectType').value;document.querySelectorAll('.lead-rental-only').forEach(function(el){el.classList.toggle('lead-hidden',!isRentalLeadCategory(category));});document.getElementById('leadNote').textContent=isRentalLeadCategory(category)?\"Les dates, le nombre de jours, le nombre de personnes et l'option chef seront inclus dans le message WhatsApp.\":\"Aucune donnee n'est stockee sur le site. Le formulaire sert uniquement a preparer votre message WhatsApp.\";}
function openLeadModal(context,amount,category){leadData.context=normalizeLeadText(context)||'Demande generale';leadData.amount=parseInt(amount,10)||0;leadData.category=normalizeLeadCategory(category);leadData.ref=makeLeadRef();document.getElementById('leadContext').textContent=leadData.context;document.getElementById('leadPrice').textContent=leadData.amount?formatLeadAmount(leadData.amount):'A confirmer';document.getElementById('leadRef').value=leadData.ref;document.getElementById('leadFirstName').value='';document.getElementById('leadLastName').value='';document.getElementById('leadPhone').value='';document.getElementById('leadEmail').value='';document.getElementById('leadAddress').value='';document.getElementById('leadProjectType').value=leadData.category;document.getElementById('leadPaymentMethod').value='Wave';document.getElementById('leadStartDate').value='';document.getElementById('leadEndDate').value='';document.getElementById('leadNights').value='';document.getElementById('leadGuests').value='';document.getElementById('leadChef').value='Oui';document.getElementById('leadMessage').value='';updateLeadRentalFields();setLeadMinDates();document.getElementById('leadModal').classList.add('open');document.body.style.overflow='hidden';}
function closeLeadModal(){document.getElementById('leadModal').classList.remove('open');document.body.style.overflow='';}
function detectLeadCategory(link,context){var block=link.closest('.vcard,.rental-card,.property-card,.ld,.long-stay,.comm,.commercial,.quick-card,.contact-card,.map-copy,.cta');var haystack=normalizeLeadText((link.textContent||'')+' '+(link.getAttribute('href')||'')+' '+(context||'')).toLowerCase();if(block&&block.matches('.vcard,.rental-card'))return 'Location vacances';if(block&&block.matches('.ld,.long-stay'))return 'Location longue duree';if(block&&block.matches('.comm,.commercial'))return 'Local commercial';if(block&&block.matches('.property-card'))return 'Achat immobilier';if(/vente|acheter|terrain|dossier/.test(haystack))return 'Achat immobilier';if(/magasin|commercial|local/.test(haystack))return 'Local commercial';if(/location|villa|vacances|sejour|conditions/.test(haystack))return 'Location vacances';return 'Renseignement general';}
function detectLeadContext(link){var block=link.closest('.vcard,.rental-card,.property-card,.ld,.long-stay,.comm,.commercial,.quick-card,.contact-card,.map-copy,.cta,.contact-section,.hero');var context='Demande generale';var amount=0;if(block){var title=block.querySelector('.vname,.card-title,.ld-title,.comm-title,.cta-title,.section-title,.contact-label,.hero-title');if(title)context=normalizeLeadText(title.textContent);var priceNode=block.querySelector('.price-a,.price,.ld-pa,.comm-price,.price-box .price');if(priceNode)amount=parseLeadAmount(priceNode.textContent);}if(context==='Demande generale'){var href=link.getAttribute('href')||'';var queryMatch=href.match(/[?&]text=([^&]+)/);if(queryMatch){context=normalizeLeadText(decodeURIComponent(queryMatch[1]).replace(/Bonjour,?/gi,''));}}return {context:context,amount:amount,category:detectLeadCategory(link,context)};}
function submitLeadWhatsApp(){var firstName=normalizeLeadText(document.getElementById('leadFirstName').value);var lastName=normalizeLeadText(document.getElementById('leadLastName').value);var phone=normalizeLeadText(document.getElementById('leadPhone').value);var email=normalizeLeadText(document.getElementById('leadEmail').value);var address=normalizeLeadText(document.getElementById('leadAddress').value);var projectType=document.getElementById('leadProjectType').value;var paymentMethod=document.getElementById('leadPaymentMethod').value;var startDate=document.getElementById('leadStartDate').value;var endDate=document.getElementById('leadEndDate').value;var nights=document.getElementById('leadNights').value;var guests=normalizeLeadText(document.getElementById('leadGuests').value);var chef=document.getElementById('leadChef').value;var message=normalizeLeadText(document.getElementById('leadMessage').value);if(!firstName||!lastName||!phone||!email||!address){alert('Veuillez remplir le prenom, le nom, le numero de telephone, l\\'adresse email et l\\'adresse.');return;}if(isRentalLeadCategory(projectType)){if(!startDate||!endDate){alert('Veuillez selectionner vos dates via le calendrier.');return;}if(!nights||parseInt(nights,10)<1){alert('La date de depart doit etre apres la date d\\'arrivee.');return;}if(!guests||parseInt(guests,10)<1){alert('Veuillez indiquer le nombre de personnes.');return;}}var lines=['Bonjour Baobab Horizon, je souhaite continuer ma demande via WhatsApp.','',`Reference: ${leadData.ref}`,`Bien concerne: ${leadData.context}`,`Type de demande: ${projectType}`];if(leadData.amount){lines.push(`Budget indicatif: ${formatLeadAmount(leadData.amount)}`);}if(isRentalLeadCategory(projectType)){lines.push(`Date d'arrivee: ${startDate}`,`Date de depart: ${endDate}`,`Nombre de jours: ${nights}`,`Nombre de personnes: ${guests}`,`Option chef cuisinier: ${chef}`);}lines.push(`Methode de paiement souhaitee: ${paymentMethod}`,'',`Prenom: ${firstName}`,`Nom: ${lastName}`,`Telephone: ${phone}`,`Email: ${email}`,`Adresse: ${address}`);if(message){lines.push('',`Informations complementaires: ${message}`);}window.open('https://wa.me/'+LEAD_WHATSAPP_NUMBER+'?text='+encodeURIComponent(lines.join('\\n')),'_blank');closeLeadModal();}
...
for path in FILES:
    ...
    if ".lead-modal{" not in text:
        text = text.replace("</style>", LEAD_CSS + "\n</style>")

    if 'id="leadModal"' not in text:
        if '<div class="pay-modal"' in text:
            text = text.replace('<div class="pay-modal"', LEAD_HTML + '\n<div class="pay-modal"', 1)
        else:
            text = text.replace("<footer>", LEAD_HTML + "\n<footer>", 1)

    if "var LEAD_WHATSAPP_NUMBER='221780140942';" not in text:
        first_script = text.find("<script>")
        if first_script != -1:
            text = text[:first_script] + LEAD_SCRIPT + "\n" + text[first_script:]
        else:
            text = text.replace("</body>", LEAD_SCRIPT + "\n</body>")
