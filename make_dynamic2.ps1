...
$startIndex = $html.IndexOf("    <article class=`"vcard`" data-type=`"vacances`">")
if ($startIndex -eq -1) {
    Write-Host "Impossible de trouver le début des villas."
    exit 1
}

# The block to remove starts just before this article (maybe we keep the first one or remove all?)
# Actually, let's just use regex or a clean loop to find the end of the section
...
$newHtml = $html.Substring(0, $startIndex) + $scriptToAdd + "`r`n  </section>`r`n  <script>`r`n" + @"
    ...
    function buildAccueilCard(slug,v){
      ...
      var reserveBtn = isVacances 
        ...
        : '<a class="btn-s" href="contact.html" style="background:transparent;color:var(--gold);border-color:var(--gold);font-weight:600;text-decoration:none;padding:13px 28px;font-size:.62rem;letter-spacing:.16em;text-transform:uppercase;">Contact</a>';
    ...
    function loadAccueil(data) {
      ...
      if(window.loadGalleryData) window.loadGalleryData(data);
      ...
      // Update global count
      if(window.filterVillas) {
         let type = document.querySelector('.filter-bar .active') ? document.querySelector('.filter-bar .active').textContent.toLowerCase() : 'all';
         if(type === 'toutes') type = 'all';
         filterVillas(type, document.querySelector('.filter-bar .active') || document.querySelector('.filter-bar .fbtn'));
      }
    }
    document.addEventListener("DOMContentLoaded", function() {
        fetch('data/properties.json?'+Date.now()).then(function(r){return r.json();}).then(loadAccueil).catch(function(err){console.log(err);});
    });
"@ + "`r`n  </script>`r`n" + $html.Substring($endIndex + 12)

# Ensure encoding is UTF8 without BOM
[System.IO.File]::WriteAllText((Join-Path (Get-Location) "index.html"), $newHtml, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "Success"
